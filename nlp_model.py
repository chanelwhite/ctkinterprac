...
def sort_array_match(string, array):
...
def random_choice_gen(input_arr, options_arr):
    my_list = []
    ...
    if random_string not in input_arr:
        ...
        return random_choice_gen(input_arr, options_arr)


def confusion_function(df):
    array_return = []
    for i in range(1):
        ...
        for j in range(2):
            test_options = random_choice_gen(test_options, joy_words)
        for j in range(2):
            test_options = random_choice_gen(test_options, spare_words)

        # let's sort them so that they appear in the same order as the scenario
        test_options = sort_array_match(scenario_string, test_options)
...
def calibration_function(df, confusion_array):
    ...
    vec_c = array_to_vec(confusion_array, model)
    ...
    for index, row in df.iterrows():
        ...
        vec_d = array_to_vec(key_words, model)
